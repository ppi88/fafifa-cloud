...
import { Users, Send, X, Clock, Calendar } from 'lucide-react';
...
const VisitInput = ({ onSave }) => {
  ...
  // 👇 STATE UNTUK KONTROL TRANSPARANSI OTOMATIS
  ...
  // 👇 FUNGSI UNTUK MENGAKTIFKAN WARNA DAN RESET TIMER 10 DETIK
  const triggerShow = () => {
    setIsActive(true);
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center px-6">
      ...
      <div className="relative bg-white dark:bg-slate-900 w-full max-w-xs rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95 duration-200">
        <button onClick={() => setIsOpen(false)} className="absolute top-6 right-6 text-slate-400 hover:text-slate-600"><X size={20}/></button>
        ...
        <div className="text-center mb-6">
          ...
          <div className="flex flex-col gap-2 mt-4">
            <div className="flex items-center justify-center gap-2 bg-slate-50 dark:bg-slate-800/50 w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all">
              <Calendar size={14} className="text-slate-400 shrink-0" />
              <input 
                type="date" 
                value={inputTanggal}
                onChange={(e) => setInputTanggal(e.target.value)}
                className="bg-transparent text-[13px] font-bold text-slate-700 dark:text-slate-200 outline-none w-full text-center"
              />
            </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between bg-slate-100 dark:bg-slate-800 p-2 rounded-2xl">
            <button onClick={() => setJumlah(Math.max(1, jumlah-1))} className="w-12 h-12 bg-white dark:bg-slate-700 rounded-xl font-black text-xl text-slate-700 dark:text-slate-200 shadow-sm active:scale-90 transition-transform">-</button>
            <span className="text-3xl font-black text-slate-800 dark:text-white">{jumlah}</span>
            <button onClick={() => setJumlah(jumlah+1)} className="w-12 h-12 bg-white dark:bg-slate-700 rounded-xl font-black text-xl text-slate-700 dark:text-slate-200 shadow-sm active:scale-90 transition-transform">+</button>
          </div>

          <button 
            onClick={handleSubmit}
            disabled={loading}
            className="w-full bg-emerald-500 py-4 rounded-2xl text-white font-black uppercase text-sm tracking-widest shadow-lg shadow-emerald-500/30 active:scale-95 transition-all flex items-center justify-center gap-2"
          >
            {loading ? "Menyimpan..." : <><Send size={18} /> Catat Kunjungan</>}
          </button>
        </div>
      </div>
    </div>
