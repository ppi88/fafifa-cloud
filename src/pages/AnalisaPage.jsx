...
import { 
  ...
  Clock, Sparkles, ArrowRight, Package, AlertCircle, Zap, Info, ShieldAlert
} from 'lucide-react';
import { useAnalisaBisnis } from '../hooks/useAnalisaBisnis';
import { generateAnalisaPrompt } from '../utils/aiPrompts'; 

// ============================================================================
// 🌍 STATIC CONFIGURATION POOL (Dibuat di luar agar Bebas Alokasi Memori Ulang)
// ============================================================================
const MONTHS_LIST = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'];
const DAYS_NAME_LIST = ['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu'];
const DONUT_COLORS_PALETTE = ['#0ea5e9', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#6366f1', '#ec4899', '#14b8a6', '#eab308'];
...
const formatPeriode = (dateStr) => {
  if (!dateStr) return "";
  const monthsShortName = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des'];
  const d = parseSafeDate(dateStr);
  if (!d || isNaN(d.getTime())) return dateStr;
  return `${String(d.getDate()).padStart(2, '0')} ${monthsShortName[d.getMonth()]} ${d.getFullYear()}`;
};

// ============================================================================
// 🧠 MEMOIZED BUSINESS INTELLIGENCE ANALYTICS PAGE
// ============================================================================
const AnalisaPage = ({ 
  archiveData = {}, 
  sisaArchive = {}, 
  ...
  const parseDateKey = useCallback((dKey) => {
    const parsed = parseSafeDate(dKey);
    return parsed || new Date(0);
  }, []);

  const allDataRangeText = useMemo(() => {
    const allDates = [...new Set([...Object.keys(archiveData), ...Object.keys(sisaArchive)])].filter(k => {
      const d = parseSafeDate(k);
      return d && !isNaN(d.getTime());
    });
    if (allDates.length === 0) return "BELUM ADA DATA";
    allDates.sort((a, b) => parseSafeDate(a) - parseSafeDate(b));
    return `${formatPeriode(parseSafeDate(allDates[0]))} - ${formatPeriode(parseSafeDate(allDates[allDates.length - 1]))}`;
  ...
  const { 
    ...
    normalizeDate, localSelectedDate: selectedDate, 
    filterMode: filterMode === 'all' ? 'periode' : filterMode, 
    selectedMonthIdx, selectedYear, 
    startDate: filterMode === 'all' ? '2020-01-01' : startDate, 
    endDate: filterMode === 'all' ? '2050-12-31' : endDate, visitData
  });

  const { productStats = {}, weeklyTargets = {}, totalLostOpportunityRp = 0 } = insightCerdas || {};

  // 🔥 OPTIMASI 1: Memoize Filter & Sorting Matriks AI agar tidak dihitung ulang setiap scroll 🔥
  const { produkBermasalah, hiddenGems, anomalies, lostOpportunityCakes, healthMatrixSorted } = useMemo(() => {
    const entries = Object.entries(productStats || {});
    
    const bermasalah = entries
    ...
    const sortedMatrix = [...entries].sort((a,b) => (b[1]?.terjual || 0) - (a[1]?.terjual || 0));

    return { 
      produkBermasalah: bermasalah, 
      hiddenGems: gems, 
      anomalies: anom, 
      lostOpportunityCakes: lostCakes,
      healthMatrixSorted: sortedMatrix
    };
  }, [productStats]);
  ...
  // 🔥 OPTIMASI 2: Memoize kalkulasi jalur SVG Grafik agar render super ringan 🔥
  const { maxTraffic, svgWidth, svgHeight, paddingY, trafficPoints, smoothLine, smoothArea } = useMemo(() => {
    const sData = trafficData?.length ? trafficData : [{ hour: 7, total: 0 }, { hour: 8, total: 0 }];
    const maxT = Math.max(...sData.map(d => d.total), 1);
    const sWidth = 800; const sHeight = 200; const pY = 40;
    const xStep = sData.length > 1 ? sWidth / (sData.length - 1) : sWidth;
    
    const tPoints = sData.map((d, i) => ({ 
      x: i * xStep, 
      y: sHeight - pY - ((d.total / maxT) * (sHeight - pY * 2)), 
      ...d 
    }));
    ...
    const sArea = tPoints.length > 1 
      ? `${sLine} L ${tPoints[tPoints.length - 1].x},${sHeight - pY} L ${tPoints[0].x},${sHeight - pY} Z` : '';

    return { maxTraffic: maxT, svgWidth: sWidth, svgHeight: sHeight, paddingY: pY, trafficPoints: tPoints, smoothLine: sLine, smoothArea: sArea };
  }, [trafficData]);
  ...
  // 🔥 OPTIMASI 3: Isolasi render loop Donut Chart dari polusi pembengkakan akumulatif 🔥
  const marketShareCirclesRender = useMemo(() => {
    let localCumulativePercent = 0;
    if (!trendData?.list) return null;

    return trendData.list.map((item, idx) => {
      const pct = trendData.totalSalesVol > 0 ? (item.terjual / trendData.totalSalesVol) : 0;
      const strokeLength = pct * DONUT_CIRCUMFERENCE;
      const offset = localCumulativePercent * DONUT_CIRCUMFERENCE;
  ...
  const handleCopyForAI = async () => {
    try {
      const periodeText = filterMode === 'all' ? allDataRangeText : filterMode === 'bulan' ? `${MONTHS_LIST[selectedMonthIdx]} ${selectedYear}` : `${formatPeriode(startDate)} - ${formatPeriode(endDate)}`;
      const textPrompt = generateAnalisaPrompt({
        ...
        productDetails: JSON.stringify(aiReadyData?.structuredProducts || [], null, 2), 
        trafficInfo: JSON.stringify(aiReadyData?.structuredTraffic || {}, null, 2), 
        weeklyTargets: JSON.stringify(aiReadyData?.structuredWeeklyTargets || {}, null, 2)
      });
      ...
      if (navigator.clipboard && window.isSecureContext) {
        ...
        textArea.value = textPrompt; document.body.appendChild(textArea); 
    ...
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-6 duration-700 pb-32 mb-8 font-sans relative bg-[#f8fafc] dark:bg-[#020617] min-h-screen w-full">
      ...
      <div className="sticky top-0 z-[70] -mx-4 sm:-mx-6 px-2 sm:px-6 pt-2 pb-3 bg-[#f8fafc]/90 dark:bg-[#020617]/90 backdrop-blur-2xl border-b border-slate-200/60 dark:border-slate-800/60 shadow-sm w-[calc(100%+2rem)] sm:w-[calc(100%+3rem)]">
        ...
        <div className="flex items-center justify-between relative z-10 mb-3 mt-1">
          <div className="flex flex-col justify-center">
            <h1 className="text-xl sm:text-2xl font-black tracking-tight text-slate-800 dark:text-white leading-none uppercase">Analisa AI</h1>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <div className="flex items-center gap-1 px-2.5 py-1.5 rounded-full border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800/60 backdrop-blur-md shadow-sm">
              ...
              <span className="text-[8px] sm:text-[9px] font-bold text-slate-800 dark:text-slate-200 whitespace-nowrap uppercase tracking-widest">
                {filterMode === 'all' ? 'ALL DATA' : filterMode === 'bulan' ? `${MONTHS_LIST[selectedMonthIdx].substring(0,3)} ${selectedYear}` : `${formatPeriode(startDate)}`}
              </span>
            </div>
            
            <button onClick={handleCopyForAI} className={`flex items-center justify-center sm:justify-start gap-2 h-7 w-7 sm:h-auto sm:w-auto sm:px-3 sm:py-1.5 rounded-xl shadow-sm active:scale-95 transition-all border ${isCopied ? 'bg-emerald-500 border-emerald-400 text-white' : 'bg-white border-slate-200 dark:bg-slate-800 dark:border-slate-700 text-slate-800 dark:text-slate-200'}`}>
              {isCopied ? <Check size={12} strokeWidth={3} /> : <Sparkles size={12} className={isCopied ? "" : "text-amber-500"} />}
              <span className="text-[9px] font-black uppercase tracking-widest hidden sm:inline">{isCopied ? 'Tersalin' : 'Copy AI Prompt'}</span>
            </button>
          </div>
        </div>
          ...
          <div className="flex flex-row gap-1.5 items-center w-full pb-1">
            <div className="bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-0.5 rounded-lg flex items-center shrink-0 shadow-sm h-[36px]">
              {['all', 'bulan', 'periode'].map(mode => (
                <button key={mode} type="button" onClick={() => setFilterMode(mode)} className={`px-3 sm:px-4 py-1.5 h-full text-[9px] font-black uppercase tracking-widest rounded-md transition-all duration-300 ${filterMode === mode ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
                  {mode === 'all' ? 'ALL' : mode === 'periode' ? 'RENTANG' : 'BULAN'}
                </button>
              ))}
            </div>
            ...
            <div className="bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-0.5 flex items-center flex-1 w-full h-[36px] shadow-sm min-w-0">
              ...
              {filterMode === 'bulan' && (
                <div className="flex items-center w-full gap-1 px-1">
                  ...
                  <div className="relative flex-1 min-w-0">
                    <select value={selectedMonthIdx} onChange={(e) => setSelectedMonthIdx(e.target.value)} className="w-full bg-white dark:bg-slate-800 text-[10px] font-bold text-slate-700 dark:text-slate-200 outline-none rounded-md border border-slate-200 dark:border-slate-700 pl-2 pr-5 py-1 appearance-none cursor-pointer truncate shadow-sm">
                      {MONTHS_LIST.map((m, i) => <option key={i} value={i}>{m}</option>)}
                    </select>
                    <ChevronDown size={10} className="absolute right-1.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                  </div>
                  <div className="relative w-[35%] min-w-[65px]">
                    <select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className="w-full bg-white dark:bg-slate-800 text-[10px] font-bold text-slate-700 dark:text-slate-200 outline-none rounded-md border border-slate-200 dark:border-slate-700 pl-2 pr-5 py-1 appearance-none cursor-pointer shadow-sm">
              ...
              {filterMode === 'periode' && (
                <div className="flex items-center w-full gap-1 px-1">
                  <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="flex-1 min-w-0 w-full bg-white dark:bg-slate-800 text-[9px] font-bold text-slate-700 dark:text-slate-200 outline-none rounded-md border border-slate-200 dark:border-slate-700 px-1 py-1 uppercase cursor-pointer text-center shadow-sm" />
                  <span className="text-slate-400 dark:text-slate-500 font-bold text-[10px] shrink-0">-</span>
                  <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="flex-1 min-w-0 w-full bg-white dark:bg-slate-800 text-[9px] font-bold text-slate-700 dark:text-slate-200 outline-none rounded-md border border-slate-200 dark:border-slate-700 px-1 py-1 uppercase cursor-pointer text-center shadow-sm" />
                </div>
              )}
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 relative z-10 mt-4 -mx-3 px-3 sm:mx-0 sm:px-0">
        <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-200/60 dark:border-blue-800/40 rounded-[1rem] p-3 shadow-sm flex flex-col justify-center transition-colors">
          <span className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-blue-600 dark:text-blue-400 mb-1"><DollarSign size={12}/> Omzet</span>
          <span className="text-sm sm:text-base font-black text-blue-900 dark:text-blue-100 truncate">{formatRp(totalOmzetGlobal)}</span>
        </div>
        
        <div className={`border rounded-[1rem] p-3 shadow-sm flex flex-col justify-center transition-colors ${(profitData?.totalLaba || 0) < 0 ? 'bg-rose-50 dark:bg-rose-900/10 border-rose-200/60 dark:border-rose-800/40' : 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-200/60 dark:border-emerald-800/40'}`}>
          <span className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest mb-1 ${(profitData?.totalLaba || 0) < 0 ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}><Activity size={12}/> Laba Bersih</span>
          <div className="flex items-end justify-between gap-1">
            <span className={`text-sm sm:text-base font-black truncate ${(profitData?.totalLaba || 0) < 0 ? 'text-rose-700 dark:text-rose-300' : 'text-emerald-700 dark:text-emerald-300'}`}>{formatRp(profitData?.totalLaba || 0)}</span>
            <span className={`text-[8px] font-bold mb-0.5 px-1.5 py-0.5 rounded-md ${(profitData?.marginPct||0) >= 40 ? 'bg-emerald-200/50 text-emerald-800 dark:bg-emerald-800/50 dark:text-emerald-200' : 'bg-amber-200/50 text-amber-800 dark:bg-amber-800/50 dark:text-amber-200'}`}>{Math.max(0, profitData?.marginPct||0).toFixed(0)}%</span>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-[1rem] p-3 shadow-sm flex items-center justify-between transition-colors">
          <div className="flex items-center justify-between w-full">
            <div className="flex flex-col w-[45%]">
              <span className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-1"><TrendingUp size={10}/> Volume</span>
              <span className="text-sm font-black text-slate-800 dark:text-white truncate">{trendData?.totalSalesVol || 0}</span>
            </div>
            <div className="h-8 w-[1px] bg-slate-200 dark:bg-slate-700"></div>
            <div className="flex flex-col text-right w-[45%]">
              <span className="text-[9px] font-black uppercase tracking-widest text-amber-500 dark:text-amber-400 mb-1">Sisa Rollover</span>
              <span className="text-sm font-black text-amber-600 dark:text-amber-400 truncate">{trendData?.totalSisa || 0}</span>
            </div>
          </div>
        </div>

        <div className="bg-purple-50 dark:bg-purple-900/10 border border-purple-200/60 dark:border-purple-800/40 rounded-[1rem] p-3 shadow-sm flex flex-col justify-center relative group cursor-pointer transition-colors" onClick={() => setShowLostTooltip(!showLostTooltip)} onMouseLeave={() => setShowLostTooltip(false)}>
          <div className="flex items-center justify-between mb-1">
            <span className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest text-purple-600 dark:text-purple-400"><TrendingUp size={10} className="rotate-180"/> Lost Opp.</span>
            <div className="bg-purple-200/50 dark:bg-purple-800/60 rounded-full p-0.5 animate-pulse"><Info size={10} className="text-purple-600 dark:text-purple-300"/></div>
          </div>
          <span className="text-sm sm:text-base font-black text-purple-700 dark:text-purple-300 truncate">{formatRp(totalLostOpportunityRp)}</span>

          <div className={`absolute top-full right-0 mt-2 w-[240px] sm:w-[280px] bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200/50 dark:border-slate-700/50 rounded-xl p-3.5 shadow-2xl transition-all duration-300 z-[100] cursor-default ${showLostTooltip ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible translate-y-2 lg:group-hover:opacity-100 lg:group-hover:visible lg:group-hover:translate-y-0'}`} onClick={(e) => e.stopPropagation()}>
            <div className="flex flex-col gap-2.5">
               <div>
                 <span className="text-[9px] font-black text-rose-500 dark:text-rose-400 uppercase tracking-widest mb-1 flex items-center gap-1"><AlertCircle size={10}/> Kenapa Terjadi?</span>
                 <p className="text-[10px] text-slate-700 dark:text-slate-300 leading-relaxed">
                   ...
                   {lostCakesNames && <span className="block mt-1.5 text-amber-600 dark:text-amber-300 font-bold bg-amber-100 dark:bg-amber-500/10 p-1.5 rounded border border-amber-200 dark:border-amber-500/20">⚠️ Sering Ludes: {lostCakesNames}</span>}
                 </p>
               </div>
               <div className="h-[1px] w-full bg-slate-200 dark:bg-slate-700/50"></div>
               <div>
                 <span className="text-[9px] font-black text-emerald-500 dark:text-emerald-400 uppercase tracking-widest mb-1 flex items-center gap-1"><Sparkles size={10}/> Saran Tindakan AI</span>
                 <p className="text-[10px] text-slate-700 dark:text-slate-300 leading-relaxed">
      ...
      <div className="-mx-3 px-3 sm:mx-0 sm:px-0 mt-6">
        <div className="bg-white/50 dark:bg-slate-900/30 backdrop-blur-2xl rounded-[1.5rem] sm:rounded-[2rem] p-4 sm:p-5 shadow-sm border border-slate-200/50 dark:border-slate-800/50 relative overflow-hidden">
          ...
          <h2 className="text-slate-800 dark:text-white text-sm font-black uppercase tracking-widest mb-4 flex items-center gap-2 relative z-10"><Sparkles size={16} className="text-sky-500 dark:text-sky-400"/> AI Action Center</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 relative z-10">
            <div className="bg-white/80 dark:bg-rose-500/10 border border-slate-200/50 dark:border-rose-500/20 shadow-sm rounded-xl p-3">
              <h3 className="text-[10px] font-black text-rose-500 dark:text-rose-400 uppercase tracking-widest mb-2 flex items-center gap-1.5"><AlertTriangle size={12}/> Pangkas (Dead Cash)</h3>
            ...
            <div className="bg-white/80 dark:bg-emerald-500/10 border border-slate-200/50 dark:border-emerald-500/20 shadow-sm rounded-xl p-3">
              <h3 className="text-[10px] font-black text-emerald-500 dark:text-emerald-400 uppercase tracking-widest mb-2 flex items-center gap-1.5"><TrendingUp size={12}/> Push Promo (Hidden Gem)</h3>
              <div className="space-y-2">
            ...
            <div className="bg-white/80 dark:bg-amber-500/10 border border-slate-200/50 dark:border-amber-500/20 shadow-sm rounded-xl p-3">
              <h3 className="text-[10px] font-black text-amber-500 dark:text-amber-400 uppercase tracking-widest mb-2 flex items-center gap-1.5"><ShieldAlert size={12}/> Anomali Terdeteksi</h3>
              <div className="space-y-2">
                  ...
                  return (
                    <div key={i} className="flex flex-col bg-slate-50/80 dark:bg-black/20 border border-slate-100 dark:border-transparent p-2 rounded-lg gap-1.5">
                      <div className="flex justify-between items-center">
                        <span className="text-[11px] font-bold text-slate-800 dark:text-white truncate max-w-[120px]">{nama}</span>
                        <span className={`text-[8px] font-black px-2 py-1 rounded-md uppercase tracking-wider ${stat?.anomaly === 'SUDDEN_DROP' ? 'bg-rose-100 text-rose-600 dark:bg-rose-500/20 dark:text-rose-300' : 'bg-sky-100 text-sky-600 dark:bg-sky-500/20 dark:text-sky-300'}`}>
        ...
        <div className="bg-white/50 dark:bg-slate-900/30 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800/50 rounded-[1.5rem] sm:rounded-[2rem] p-4 sm:p-5 shadow-sm flex flex-col relative z-10">
          <div className="flex items-center justify-between mb-4 px-1">
            <div className="flex items-center gap-2.5">
              <div className="bg-indigo-500/10 p-1.5 rounded-lg"><Package size={16} className="text-indigo-600 dark:text-indigo-400" /></div>
              <div>
                <h3 className="text-[13px] font-black text-slate-800 dark:text-slate-100 leading-none mb-0.5">Target Produksi AI</h3>
                <p className="text-[9px] text-slate-500 font-medium">Geser untuk melihat hari lain.</p>
              </div>
            </div>
          </div>
          
          <div className="flex gap-3 sm:gap-4 overflow-x-auto custom-scrollbar-hide pb-2 snap-x snap-mandatory -mx-1 px-1">
            {DAYS_NAME_LIST.map((day, idx) => {
              ...
              return (
                <div key={idx} className="min-w-[280px] sm:min-w-[340px] shrink-0 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-[1.2rem] overflow-hidden shadow-sm flex flex-col snap-start">
                  ...
                  <div className="divide-y divide-slate-100 dark:divide-slate-800/50 flex-1 max-h-[350px] overflow-y-auto custom-scrollbar p-1">
                    {items.length === 0 ? <div className="p-8 text-center text-slate-400"><p className="text-[10px] font-bold">Belum ada data cukup</p></div> : (
                      items.map((item, i) => {
                        const isNaik = item.type === 'naik'; const isTurun = item.type === 'turun';
                        ...
                        return (
                          <div key={i} className={`p-2.5 flex items-center justify-between gap-2 transition-colors ${bgRow}`}>
                            ...
                            <div className="flex flex-col items-end shrink-0 w-[65px] sm:w-[70px] border-r border-slate-200 dark:border-slate-700 pr-2">
                               ...
                               <span className="text-[8px] font-black text-sky-500 mt-0.5 cursor-help" title={`Trend: ${item.explain?.factors?.trend}\nTraffic: ${item.explain?.factors?.traffic}`}>
                ...
                {trendData?.list?.map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between mb-1.5 border-b border-slate-100/50 dark:border-slate-800/50 pb-1">
                    <div className="flex items-center gap-1.5 overflow-hidden">
                      <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: DONUT_COLORS_PALETTE[idx % DONUT_COLORS_PALETTE.length] }}></span>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#0b1120] backdrop-blur-xl rounded-[1.5rem] sm:rounded-[2.5rem] p-4 sm:p-6 border border-slate-200/60 dark:border-slate-800 shadow-sm relative overflow-hidden flex flex-col">
          ...
          <div className="h-56 w-full mt-2 overflow-x-auto [&::-webkit-scrollbar]:hidden -mx-2 px-2">
            <svg viewBox={`0 0 ${svgWidth} ${svgHeight}`} className="w-full min-w-[550px] h-full overflow-visible">
              ...
              {[0, 0.33, 0.66, 1].map(r => (
                <line key={r} x1="0" y1={svgHeight - paddingY - (r * (svgHeight - paddingY * 2))} x2={svgWidth} y2={svgHeight - paddingY - (r * (svgHeight - paddingY * 2))} stroke="currentColor" className="text-slate-200 dark:text-[#334155]" strokeOpacity="0.5" strokeWidth="1" strokeDasharray="5,5" />
              ))}
              ...
              {trafficPoints.map((p, i) => {
                const hasTrafficData = p.total > 0;
                return (
                  <g key={i}>
                    {hasTrafficData && <line x1={p.x} y1={p.y} x2={p.x} y2={svgHeight - paddingY} stroke="#f59e0b" strokeOpacity="0.3" strokeWidth="1" strokeDasharray="4 4" />}
                    {hasTrafficData && <circle cx={p.x} cy={p.y} r="5" className="fill-white dark:fill-slate-900" stroke="#f59e0b" strokeWidth="3" />}
                    {hasTrafficData && (
                    ...
                    <text x={p.x} y={svgHeight - paddingY + 22} textAnchor="middle" fontSize="10" fontWeight="bold" className="fill-slate-500 dark:fill-slate-400">{String(p.hour).padStart(2, '0')}:00</text>
                  </g>
                )
              })}
            </svg>
              ...
              {dayTrendData?.list?.map((item, idx) => {
                ...
                return (
                  <div key={idx} className="flex flex-col items-center justify-end h-full flex-1 group">
                    ...
                    <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 transition-all opacity-0 scale-90 group-hover:opacity-100 group-hover:scale-100 absolute -top-5 bg-slate-800 text-white rounded px-1.5 py-0.5 z-20 pointer-events-none">{item.avg} pcs</span>
                  </div>
                );
              })}
            </div>
...
export default React.memo(AnalisaPage);